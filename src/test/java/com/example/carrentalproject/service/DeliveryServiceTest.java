...
//Setup:
//
//        Mocks: Uses Mockito to create mock objects for CarRepository, OrderRepository, AccessKeyRepository, and LoggedInUser.
//        Inject Mocks: DeliveryService is instantiated with the mocked dependencies.
//        Test Methods:
//
                //        itShouldRentACar(): Verifies that when a car is available and a user has a valid access key, the car is rented out successfully, and its availability is updated. The access key is also deleted.
                //        itShouldThrowEntityNotFoundException(): Ensures that attempting to rent a non-existent car throws an EntityNotFoundException.
                //        itShouldThrowNoAccessKeyException(): Tests that renting a car without an access key throws a NoAccessKeyException.
                //        itShouldThrowInvalidPackageException(): Validates that if the car package in the access key does not match the car's package, an InvalidPackageException is thrown.
                //        itShouldThrowUnavailableCarException(): Confirms that trying to rent a car that is not available throws an UnavailableCarException.